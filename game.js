...
(function () {
  ...
  var UNLIMITED_REROLLS = { region: Infinity, team: Infinity, year: Infinity };
  var UNLIMITED_MSI_REROLLS = { team: Infinity, year: Infinity };
  ...
  var state = {
    ...
    used: new Set(),
    rerolls: { region: Infinity, team: Infinity, year: Infinity },
    spin: null,
    spinning: false,
    result: null,
    teamRating: null
  ...
  function cloneRerolls(map) {
    return Object.assign({}, map);
  }
  function rerollLabel(n) {
    return Number.isFinite(n) ? String(n) : "∞";
  }
  function spendReroll(kind) {
    if (Number.isFinite(state.rerolls[kind])) state.rerolls[kind]--;
  }
  ...
  function startRun(mode) {
    ...
    state.used = new Set();
    state.rerolls = isMSI() ? cloneRerolls(UNLIMITED_MSI_REROLLS) : cloneRerolls(UNLIMITED_REROLLS);
    state.result = null;
  ...
  function renderDraft() {
    ...
    function rrBtn(kind, label) {
      ...
      return '<button class="reroll-btn" data-action="reroll" data-kind="' + kind + '"' + dis + '>' +
        '🎲 Reroll ' + label + ' <span class="rr-count">' + rerollLabel(n) + '</span></button>';
    }
    ...
    app.innerHTML =
      '<div class="screen draft">' +
        ...
        '<div class="reroll-note">Unlimited Region · Team · Year rerolls</div>' +
        candHtml +
      '</div>';
    fitWheelValues();
  }
  ...
  function renderDraftMSI() {
    ...
    app.innerHTML =
      '<div class="screen draft msi">' +
        ...
        '<div class="reroll-note">Unlimited Team · Year rerolls in MSI Challenge</div>' +
        candHtml +
      '</div>';
    fitWheelValues();
  }
      ...
      '<div class="screen start">' +
        ...
        '<div class="howto">' +
          ...
          '<ol class="howto-steps">' +
            ...
            '<li>You can reroll <b>Region, Team and Year without limits</b> whenever another valid option exists.</li>' +
          '</ol>' +
        '</div>' +
        '<div class="event-banner">' +
          '<span class="eb-badge">NEW · LIMITED EVENT</span>' +
          '<span class="eb-title">🔥 MSI Challenge</span>' +
          '<span class="eb-sub">Tap <b>MSI Challenge</b> to spin memorable <b>Mid-Season Invitational team-years</b>, draft one player per position, then run the bracket: <b>Play-In → Main Stage → Grand Final.</b> Team and Year rerolls are unlimited.</span>' +
        '</div>' +
        '<section class="publisher-content" aria-label="Golden Road LoL guide">' +
          ...
          '<div class="content-section">' +
            ...
            '<ul class="content-list">' +
              '<li><b>Use rerolls freely to hunt better team-years.</b> A Team reroll can rescue a difficult region-year, while a Year reroll is great when the organization had one legendary season nearby.</li>' +
